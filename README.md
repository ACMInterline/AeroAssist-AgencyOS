...
- Disabled coming-soon sidebar entries for Website/CMS and Offers/Pricing.
...
Phase 28.1 adds app-shell and visual polish stabilization, but it is not a complete operations stack. Production use still requires migrations, off-server backup policy, alerting decisions, broader automated tests, provider webhook/bounce handling decisions, real object-storage integration decisions, automatic invite delivery decisions, broader team management, section-level post-create editing, visual regression automation, and deeper request workspace hardening.
