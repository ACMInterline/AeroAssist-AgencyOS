...
The current build marker is `phase_58_2_commercial_pilot_operations_command_centre_foundation`. Phase 58.2 makes `/agency` the consultant's operational home after onboarding. It aggregates existing tenant-owned work, queues, deadlines, timeline events, alerts, quick actions, and recent activity, and delegates supported assignment and completion actions to the canonical work queue. It creates no parallel operational records or provider execution. See [Commercial Pilot Operations Command Centre Foundation](docs/architecture/commercial-pilot-operations-command-centre-foundation.md).
