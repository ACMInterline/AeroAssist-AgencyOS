...
The current build marker is `phase_58_1_commercial_pilot_agency_onboarding_foundation`. Phase 58.1 adds a resumable first-time setup wizard for newly created agencies, canonical branding/template/preference defaults, and an idempotent synthetic airline-travel workspace. Existing agencies are not retroactively forced through onboarding, and the seed performs no provider, payment, ticketing, messaging, or external operation. See [Commercial Pilot Agency Onboarding Foundation](docs/architecture/commercial-pilot-agency-onboarding-foundation.md).
