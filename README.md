...
The current build marker is `phase_58_3_complete_pilot_agency_experience`. Phase 58.3 expands the explicit onboarding demo action into a selectable, deterministic, tenant-scoped pilot workspace spanning canonical clients, passengers, requests, trips, offers, bookings, ticket/EMD mirrors, passenger services, documents, finance, workflow, timeline, and after-sales records. The linked dataset naturally populates the Phase 58.2 Operations Command Centre and performs no provider, payment, communication, or ticketing execution. See [Complete Pilot Agency Experience](docs/architecture/complete-pilot-agency-experience.md).
