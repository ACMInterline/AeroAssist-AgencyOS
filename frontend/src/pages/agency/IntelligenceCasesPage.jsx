...
export default function IntelligenceCasesPage() {
  ...
  async function load(nextFilters = filters) {
    ...
    const response = await apiGet(`/api/agencies/${context.agency.id}/intelligence-cases${query}`)
    setState({ ...context, items: response.items || response.cases || [], summary: response.summary || {} })
  }

  useEffect(() => {
    load(filters).catch((err) => setError(err.message))
  }, [filters.case_status, filters.overall_case_status, filters.airline, filters.passenger_need, filters.travel_request, filters.trip_workspace, filters.ready_for_agent_review, filters.ready_for_offer_builder, filters.ready_for_client_presentation])
        ...
        <div className="space-y-6">
          <div className="flex flex-wrap items-start justify-between gap-3">
            <div>
              ...
              <h2 className="mt-2 text-2xl font-semibold text-slate-950">Intelligence Cases</h2>
              <p className="mt-1 text-sm text-slate-600">Read-only agency view of the Chapter 50 operational intelligence chain, from passenger need to offer-intelligence package. It adds no new intelligence and keeps all final decisions with human reviewers.</p>
            </div>
            <div className="flex flex-wrap gap-2">
              <span className="rounded-full bg-blue-50 px-3 py-2 text-xs font-semibold text-blue-700">Pipeline view</span>
          ...
          <section className="space-y-3">
            <div className="flex items-center justify-between gap-3">
              <h3 className="text-lg font-semibold text-slate-950">Intelligence Cases</h3>
              <span className="rounded-full bg-slate-100 px-3 py-1 text-xs font-semibold text-slate-600">{items.length}</span>
            </div>
            {items.length ? <CaseList items={items} /> : <EmptyState title="No intelligence cases" body="Operational intelligence case metadata for this agency will appear here." />}
          </section>
        </div>
      </ProtectedRoute>
    </AgencyLayout>
        ...
        <details key={item.id} className="rounded-lg border border-slate-200 bg-white p-4" open>
          <summary className="cursor-pointer list-none">
            <div className="grid gap-3 md:grid-cols-[1fr_180px_180px]">
              ...
              <p className="text-xs text-slate-600">Request: {item.travel_request_id || "Unset"}</p>
              <p className="text-xs text-slate-600">Offer builder: {item.ready_for_offer_builder ? "Ready" : "Not ready"}</p>
            </div>
          </summary>
