...
export default function OperationalConstraintsPage() {
  ...
  async function load(nextFilters = filters) {
    ...
    const response = await apiGet(`/api/agencies/${context.agency.id}/operational-constraints${query}`)
    setState({ ...context, constraints: response.items || [], summary: response.summary || {} })
  }

  useEffect(() => {
    load(filters).catch((err) => setError(err.message))
  }, [filters.acquisition_id, filters.airline, filters.service_domain, filters.service_family, filters.ssr_code, filters.rfic, filters.rfisc, filters.constraint_status, filters.outcome_type, filters.review_status, filters.approval_status, filters.evaluation_ready])
        ...
        <div className="space-y-6">
          <div className="flex flex-wrap items-start justify-between gap-3">
            <div>
              ...
              <p className="mt-1 text-sm text-slate-600">Read-only AOIE constraint language metadata for your agency. It shows condition groups, outcomes, applicability, precedence, governance, and future evaluation notes without live rule execution, AI reasoning, recommendations, feasibility scoring, pricing calculation, parser execution, scraping, workers, or providers.</p>
            </div>
          ...
          {state?.constraints?.length ? <ConstraintList constraints={state.constraints} /> : <EmptyState title="No operational constraints" body="Operational constraint metadata shared with this agency will appear here." />}
        </div>
      </ProtectedRoute>
    </AgencyLayout>
  )
          ...
          <summary className="cursor-pointer list-none">
            <div className="grid gap-3 md:grid-cols-[1fr_180px_180px]">
              ...
              <div className="text-xs text-slate-600">
                <p>Status: {formatType(item.constraint_status)}</p>
                <p className="mt-1">Review: {formatType(item.review_status)}</p>
              </div>
              <div className="text-xs text-slate-600">
                <p>Approval: {formatType(item.approval_status)}</p>
                <p className="mt-1">Ready metadata: {item.evaluation_ready ? "Yes" : "No"}</p>
              </div>
            </div>
          </summary>
          <ConstraintSections item={item} />
        </details>
      ))}
    </section>
  )
...
function ConstraintSections({ item }) {
  return (
