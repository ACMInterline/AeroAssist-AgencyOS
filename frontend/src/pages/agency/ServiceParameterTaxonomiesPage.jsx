...
export default function ServiceParameterTaxonomiesPage() {
  ...
  async function load(nextFilters = filters) {
    ...
    const response = await apiGet(`/api/agencies/${context.agency.id}/service-parameter-taxonomies${query}`)
    setState({ ...context, items: response.items || response.taxonomies || [], summary: response.summary || {} })
  }

  useEffect(() => {
    load(filters).catch((err) => setError(err.message))
  }, [filters.taxonomy_status, filters.policy_family, filters.service_family, filters.service_code, filters.parameter_domain, filters.parameter_group, filters.parameter_scope, filters.review_status, filters.approval_status])
        ...
        <div className="space-y-6">
          <div className="flex flex-wrap items-start justify-between gap-3">
            <div>
              ...
              <p className="mt-1 text-sm text-slate-600">Reusable measurable service parameters for agency operational intelligence metadata. They describe fields for structured review; they do not evaluate rules, calculate prices, book, ticket, call providers, or automate recommendations.</p>
            </div>
            <div className="flex flex-wrap gap-2">
              <span className="rounded-full bg-blue-50 px-3 py-2 text-xs font-semibold text-blue-700">Parameter layer</span>
          ...
          <section className="space-y-3">
            <div className="flex items-center justify-between gap-3">
              <h3 className="text-lg font-semibold text-slate-950">Service Parameter Taxonomies</h3>
              <span className="rounded-full bg-slate-100 px-3 py-1 text-xs font-semibold text-slate-600">{items.length}</span>
            </div>
            {items.length ? <TaxonomyList items={items} /> : <EmptyState title="No service parameter taxonomies" body="Reusable parameter taxonomy metadata for this agency will appear here." />}
          </section>
        </div>
      </ProtectedRoute>
    </AgencyLayout>
        ...
        <details key={item.id} className="rounded-lg border border-slate-200 bg-white p-4" open>
          <summary className="cursor-pointer list-none">
            <div className="grid gap-3 md:grid-cols-[1fr_180px_180px]">
              ...
              <p className="text-xs text-slate-600">Status: {formatType(item.taxonomy_status)}</p>
              <p className="text-xs text-slate-600">Approval: {formatType(item.approval_status)}</p>
            </div>
          </summary>
