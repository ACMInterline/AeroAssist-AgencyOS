...
export default function RolloutRisksPage() {
  ...
  async function load(nextFilters = filters) {
    ...
    const [risks, summary] = await Promise.all([
      apiGet(`/api/agencies/${context.agency.id}/feature-bundle-rollout-risks${query}`),
      apiGet(`/api/agencies/${context.agency.id}/feature-bundle-rollout-risks/summary`),
    ])
    setState({
      ...context,
      risks: risks.items || [],
      summary: risks.summary || summary.summary || {},
    })
  }

  useEffect(() => {
    load(filters).catch((err) => setError(err.message))
  }, [filters.bundle_id, filters.rollout_plan_id, filters.status, filters.impact, filters.likelihood])
        ...
        <div className="space-y-6">
          <div className="flex flex-wrap items-start justify-between gap-3">
            <div>
              ...
              <h2 className="mt-2 text-2xl font-semibold text-slate-950">Rollout Risks</h2>
              <p className="mt-1 text-sm text-slate-600">Read-only rollout risk metadata. These records do not execute rollouts, enforce risk decisions, block anything, send notifications, activate bundles, add automation, or call external providers.</p>
            </div>
          ...
          {state?.risks?.length ? <RiskTable risks={state.risks} /> : <EmptyState title="No rollout risks" body="Platform risk metadata will appear here after rollout risk records are created." />}
        </div>
      </ProtectedRoute>
    </AgencyLayout>
  )
