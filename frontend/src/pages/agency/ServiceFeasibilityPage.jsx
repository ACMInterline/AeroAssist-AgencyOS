...
export default function ServiceFeasibilityPage() {
  ...
  async function load(nextFilters = filters) {
    ...
    const response = await apiGet(`/api/agencies/${context.agency.id}/passenger-service-feasibility${query}`)
    setState({ ...context, items: response.items || response.feasibilities || [], summary: response.summary || {} })
  }

  useEffect(() => {
    load(filters).catch((err) => setError(err.message))
  }, [filters.feasibility_status, filters.feasibility_type, filters.airline, filters.feasibility_outcome, filters.confidence_level, filters.operational_risk, filters.passenger_need_category, filters.ssr_code, filters.travel_date, filters.cabin, filters.destination, filters.recommendation_ready])
        ...
        <div className="space-y-6">
          <div className="flex flex-wrap items-start justify-between gap-3">
            <div>
              ...
              <h2 className="mt-2 text-2xl font-semibold text-slate-950">Service Feasibility</h2>
              <p className="mt-1 text-sm text-slate-600">Read-only advisory feasibility metadata. Feasibility is not Boolean, not recommendation, and human authority remains final. This view does not rank airlines, search flights, book, ticket, use AI or LLM prompts, call providers, execute parsers, optimise pricing, or run workers.</p>
            </div>
          ...
          <section className="space-y-3">
            ...
            {items.length ? <FeasibilityList items={items} /> : <EmptyState title="No service feasibility records" body="Passenger service feasibility metadata visible to this agency will appear here." />}
          </section>
        </div>
      </ProtectedRoute>
    </AgencyLayout>
...
function FeasibilityList({ items }) {
  return (
        ...
        <details key={item.id} className="rounded-lg border border-slate-200 bg-white p-4" open>
          <summary className="cursor-pointer list-none">
            <div className="grid gap-3 md:grid-cols-[1fr_180px_180px]">
              ...
              <p className="text-xs text-slate-600">Status: {formatType(item.feasibility_status)}</p>
