...
const capabilityStatuses = ["draft", "under_review", "approved", "active", "superseded", "available", "unavailable", "conditional", "restricted", "manual_review", "unknown"]
        ...
        <div className="space-y-6">
          <div className="flex flex-wrap items-start justify-between gap-3">
            <div>
              ...
              <h2 className="mt-2 text-2xl font-semibold text-slate-950">Capability Matrix</h2>
              <p className="mt-1 text-sm text-slate-600">Read-only metadata inventory of what airlines can operationally deliver. Capability is different from policy; this page does not evaluate passenger cases, score feasibility, rank airlines, reason with AI, execute parsers, calculate pricing, call providers, run workers, or automatically publish.</p>
            </div>
          ...
          <section className="space-y-3">
            ...
            {items.length ? <CapabilityList items={items} /> : <EmptyState title="No capability matrix records" body="Airline operational capability metadata visible to this agency will appear here." />}
          </section>
        </div>
      </ProtectedRoute>
    </AgencyLayout>
        ...
        <details key={item.id} className="rounded-lg border border-slate-200 bg-white p-4" open>
          <summary className="cursor-pointer list-none">
            <div className="grid gap-3 md:grid-cols-[1fr_180px_180px]">
              ...
              <p className="text-xs text-slate-600">Status: {formatType(item.capability_status)}</p>
          ...
          <div className="mt-4 grid gap-3 text-xs text-slate-600 lg:grid-cols-2">
            <RecordCard title="Capability Overview" value={{ capability_reference: item.capability_reference, capability_version: item.capability_version, capability_name: item.capability_name, capability_description: item.capability_description, capability_type: item.capability_type, capability_status_value: item.capability_status_value, capability_outcome: item.capability_outcome, capability_reason: item.capability_reason }} />
            <RecordCard title="Airline" value={{ airline_code: item.airline_code, airline_name: item.airline_name, validating_carrier: item.validating_carrier, operating_carrier: item.operating_carrier, marketing_carrier: item.marketing_carrier }} />
            ...
            <RecordCard title="Notes" value={{ operational_notes: item.operational_notes, internal_notes: item.internal_notes }} />
          </div>
        </details>
      ))}
    </div>
