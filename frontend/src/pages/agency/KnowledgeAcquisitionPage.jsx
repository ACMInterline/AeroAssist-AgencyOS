...
export default function KnowledgeAcquisitionPage() {
  ...
  async function load(nextFilters = filters) {
    ...
    const response = await apiGet(`/api/agencies/${context.agency.id}/airline-knowledge-acquisition${query}`)
    setState({ ...context, acquisitions: response.items || [], summary: response.summary || {}, futureFeeds: response.future_aoie_feeds || [] })
  }

  useEffect(() => {
    load(filters).catch((err) => setError(err.message))
  }, [filters.airline, filters.service_domain, filters.service_family, filters.ssr_code, filters.rfic, filters.rfisc, filters.source_type, filters.review_status, filters.approval_status, filters.effective_date, filters.official_source_flag])
        ...
        <div className="space-y-6">
          <div className="flex flex-wrap items-start justify-between gap-3">
            <div>
              ...
              <h2 className="mt-2 text-2xl font-semibold text-slate-950">Knowledge Acquisition</h2>
              <p className="mt-1 text-sm text-slate-600">Read-only airline source evidence metadata and Airline Operational Knowledge Graph metadata for your agency across evidence, policy, pricing, capability, and operational constraints. This page does not parse, interpret with AI, scrape, crawl, automate airline websites, call providers or live airline APIs, recommend, assess feasibility, calculate prices, or run background workers.</p>
            </div>
            <div className="flex flex-wrap gap-2">
              <span className="rounded-full bg-blue-50 px-3 py-2 text-xs font-semibold text-blue-700">Read-only</span>
              <span className="rounded-full bg-slate-100 px-3 py-2 text-xs font-semibold text-slate-700">Operational knowledge graph</span>
            </div>
          </div>
          ...
          <section className="rounded-lg border border-slate-200 bg-white p-5">
            <h3 className="font-semibold text-slate-950">Evidence filters</h3>
          ...
          {state?.acquisitions?.length ? <AcquisitionList acquisitions={state.acquisitions} /> : <EmptyState title="No knowledge acquisition records" body="Airline source evidence metadata shared with this agency will appear here." />}
        </div>
      </ProtectedRoute>
    </AgencyLayout>
  )
...
function AcquisitionList({ acquisitions }) {
  return (
        ...
        <details key={item.id} className="rounded-lg border border-slate-200 bg-white p-4" open>
          <summary className="cursor-pointer list-none">
            <div className="grid gap-3 md:grid-cols-[1fr_180px_180px]">
              ...
              <div className="text-xs text-slate-600">
                <p>Effective: {formatDate(item.source_effective_date)}</p>
                <p className="mt-1">Official: {item.official_source_flag ? "Yes" : "No"}</p>
              </div>
            </div>
          </summary>
          <KnowledgeGraphSections item={item} />
        </details>
      ))}
    </section>
  )
...
function KnowledgeGraphSections({ item }) {
  return (
