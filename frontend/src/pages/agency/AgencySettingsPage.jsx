  ...
  return (
    <AgencyLayout user={state?.me?.user} agency={previewAgency || state?.agency}>
      <ProtectedRoute loading={!state && !error} error={error}>
        {state?.agency ? (
          <form className="grid gap-6 lg:grid-cols-[1.1fr_0.9fr]" onSubmit={save}>
            <section className="rounded-lg border border-slate-200 bg-white p-6">
              <div className="flex flex-wrap items-start justify-between gap-3">
                <div>
                  <p className="text-sm font-semibold uppercase tracking-wide text-blue-700">Agency Settings</p>
                  <h2 className="mt-2 text-2xl font-semibold text-slate-950">Branding and theme</h2>
                  <p className="mt-2 text-sm text-slate-600">Personalize the workspace through controlled presets, not arbitrary CSS.</p>
                </div>
                <div className="flex gap-2">
                  <button className="rounded-md border border-slate-200 px-3 py-2 text-sm text-slate-700" type="button" onClick={resetTheme}>Reset</button>
                  <button className="rounded-md bg-blue-600 px-3 py-2 text-sm font-medium text-white" type="submit">Save</button>
                </div>
              </div>
              ...
              <div className="mt-6 grid gap-5">
                ...
                <div className="rounded-lg border border-slate-200 p-4">
                  <h3 className="text-sm font-semibold text-slate-900">Brand logo</h3>
                  <p className="mt-1 text-sm text-slate-600">PNG, JPEG, or WEBP only. Max 2MB. SVG is blocked for safety.</p>
                  <div className="mt-4 flex flex-wrap items-center gap-4">
                    {form.logo_url ? <img className="h-16 w-16 rounded-md border object-contain p-1" src={form.logo_url} alt="Current agency logo" /> : <div className="flex h-16 w-16 items-center justify-center rounded-md bg-slate-100 text-sm text-slate-500">No logo</div>}
