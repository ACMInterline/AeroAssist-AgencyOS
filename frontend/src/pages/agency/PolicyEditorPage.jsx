...
export default function PolicyEditorPage() {
  ...
  async function load(nextFilters = filters) {
    ...
    const response = await apiGet(`/api/agencies/${context.agency.id}/policy-editor${query}`)
    setState({ ...context, items: response.items || response.policy_cards || [], summary: response.summary || {} })
  }

  useEffect(() => {
    load(filters).catch((err) => setError(err.message))
  }, [filters.airline, filters.policy_family, filters.service_family, filters.service_code, filters.status, filters.support_status, filters.search])
        ...
        <div className="space-y-6">
          <div className="flex flex-wrap items-start justify-between gap-3">
            <div>
              ...
              <h2 className="mt-2 text-2xl font-semibold text-slate-950">Policy Editor</h2>
              <p className="mt-1 text-sm text-slate-600">Agency policy-card metadata with no-code sections for airline service policies, evidence, governance, and taxonomy links.</p>
            </div>
          ...
          <section className="space-y-3">
            ...
            {items.length ? <PolicyCardList items={items} /> : <EmptyState title="No policy cards" body="Policy Editor metadata for this agency will appear here." />}
          </section>
        </div>
      </ProtectedRoute>
    </AgencyLayout>
        ...
        <details key={item.id} className="rounded-lg border border-slate-200 bg-white p-4" open>
          <summary className="cursor-pointer list-none">
            <div className="grid gap-3 md:grid-cols-[1fr_170px_170px]">
              ...
              <p className="text-xs text-slate-600">Family: {formatType(item.policy_family)}</p>
              <p className="text-xs text-slate-600">Support: {formatType(item.support_status)}</p>
            </div>
          </summary>
