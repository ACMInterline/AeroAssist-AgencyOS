...
export default function TravelWorkspacesPage() {
  ...
  async function load(nextFilters = filters) {
    ...
    const [workspaces, summary] = await Promise.all([
      apiGet(`/api/agencies/${context.agency.id}/operational-travel-workspaces${query}`),
      apiGet(`/api/agencies/${context.agency.id}/operational-travel-workspaces/summary`),
    ])
    setState({
      ...context,
  ...
  }, [filters.status, filters.workspace_type, filters.priority, filters.assigned_agent, filters.travel_date])
        ...
        <div className="space-y-6">
          <div className="flex flex-wrap items-start justify-between gap-3">
            <div>
              ...
              <h2 className="mt-2 text-2xl font-semibold text-slate-950">Travel Workspaces</h2>
              <p className="mt-1 text-sm text-slate-600">Read-only operational travel workspace metadata. These records do not execute bookings, issue tickets, connect to live GDS or NDC, process payments, send email or SMS, run AI automation, call external APIs, integrate suppliers, call live airlines, run background workers, or automate travel operations.</p>
            </div>
          ...
          {state?.workspaces?.length ? <WorkspaceList workspaces={state.workspaces} /> : <EmptyState title="No travel workspaces" body="Operational travel workspace metadata will appear here after platform records are created for this agency." />}
        </div>
      </ProtectedRoute>
    </AgencyLayout>
  )
