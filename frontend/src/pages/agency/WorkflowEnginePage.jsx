...
export default function WorkflowEnginePage() {
  ...
  async function load(nextFilters = filters) {
    ...
    const [workflows, summary] = await Promise.all([
      apiGet(`/api/agencies/${context.agency.id}/passenger-service-workflows${query}`),
      apiGet(`/api/agencies/${context.agency.id}/passenger-service-workflows/summary`),
    ])
    setState({
      ...context,
      workflows: workflows.items || [],
      summary: workflows.summary || summary.summary || {},
    })
  }

  useEffect(() => {
    load(filters).catch((err) => setError(err.message))
  }, [filters.stage, filters.readiness, filters.passenger, filters.airline, filters.priority, filters.assigned_agent])
        ...
        <div className="space-y-6">
          <div className="flex flex-wrap items-start justify-between gap-3">
            <div>
              ...
              <h2 className="mt-2 text-2xl font-semibold text-slate-950">Workflow Engine</h2>
              <p className="mt-1 text-sm text-slate-600">Read-only passenger service workflow metadata coordinating passenger, request, trip, booking, ticket, EMD, SSR / OSI, document, timeline, and future AOIE references. It does not automate workflows, make AI decisions, approve, ticket, issue EMDs, message, run workers, or call providers.</p>
            </div>
          ...
          {state?.workflows?.length ? <WorkflowList workflows={state.workflows} /> : <EmptyState title="No passenger service workflows" body="Workflow coordination metadata will appear here after platform records are created for this agency." />}
        </div>
      </ProtectedRoute>
    </AgencyLayout>
  )
      ...
      {workflows.map((workflow) => (
        <details key={workflow.id} className="rounded-lg border border-slate-200 bg-white p-4" open>
          <summary className="cursor-pointer list-none">
            <div className="grid gap-3 md:grid-cols-[1fr_180px_180px]">
              ...
              <div className="text-xs text-slate-600">
                <p>Priority: {workflow.workflow_priority || "Unset"}</p>
                <p className="mt-1">Agent: {workflow.responsible_agent || "Unset"}</p>
              </div>
              <div className="text-xs text-slate-600">
                <p>Airline: {workflow.related_airline || "Unset"}</p>
                <p className="mt-1">Team: {workflow.responsible_team || "Unset"}</p>
              </div>
            </div>
          </summary>
          <div className="mt-4 grid gap-4 text-xs text-slate-600 lg:grid-cols-4">
            ...
            <DetailBlock title="Future AOIE" lines={[
              `Recommendation pack: ${workflow.recommendation_pack_reference || "Unset"}`,
              `Type: ${workflow.workflow_type || "Unset"}`,
              `Version: ${workflow.workflow_version || "Unset"}`,
              `Status: ${workflow.workflow_status || "Unset"}`,
            ]} />
