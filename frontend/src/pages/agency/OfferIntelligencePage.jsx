...
export default function OfferIntelligencePage() {
  ...
  async function load(nextFilters = filters) {
    ...
    const response = await apiGet(`/api/agencies/${context.agency.id}/offer-intelligence${query}`)
    setState({ ...context, items: response.items || response.packages || [], summary: response.summary || {} })
  }

  useEffect(() => {
    load(filters).catch((err) => setError(err.message))
  }, [filters.package_status, filters.airline, filters.recommendation_level, filters.readiness_status, filters.operational_risk, filters.passenger_need, filters.destination, filters.travel_date, filters.offer_workspace, filters.client_visibility_status])
      ...
      <ProtectedRoute loading={!state && !error} error={error}>
        <div className="space-y-6">
          <div className="flex flex-wrap items-start justify-between gap-3">
            <div>
              <p className="text-sm font-semibold uppercase tracking-wide text-blue-700">Offer Builder</p>
              <h2 className="mt-2 text-2xl font-semibold text-slate-950">Offer Intelligence</h2>
              <p className="mt-1 text-sm text-slate-600">Human-reviewed offer-intelligence metadata prepared from recommendations, feasibility, operational evaluations, capability matrix records, and evidence. No automatic offer sending, live search, booking, ticketing, EMD issuance, provider calls, or AI generation.</p>
            </div>
          ...
          <section className="space-y-3">
            ...
            {items.length ? <PackageList items={items} /> : <EmptyState title="No offer intelligence packages" body="Offer intelligence metadata for this agency will appear here." />}
          </section>
        </div>
      </ProtectedRoute>
    </AgencyLayout>
        ...
        <details key={item.id} className="rounded-lg border border-slate-200 bg-white p-4" open>
          <summary className="cursor-pointer list-none">
            <div className="grid gap-3 md:grid-cols-[1fr_180px_180px]">
              ...
              <p className="text-xs text-slate-600">Offer: {item.offer_reference || item.offer_workspace_id || "Unset"}</p>
              <p className="text-xs text-slate-600">Status: {formatType(item.package_status)}</p>
            </div>
          </summary>
