...
// Internal compatibility labels: Apply to canonical Journey; Prepare client comparison.
...
export default function JourneyAuthoringWorkspacePage() {
  ...
  async function createSession() {
    ...
    const response = await apiPost(`/api/agencies/${state.agency.id}/journey-authoring`, { title: "New itinerary draft", ...source })
    ...
    setNotice("Itinerary draft created. No production itinerary record has been changed.")
    setSelectedSegments([])
    setPreview(null)
  ...
  async function applyJourney() {
    ...
    setNotice(`Applied through the canonical itinerary service. ${response.created_record_ids?.length || 0} itinerary records created; no snapshot was published.`)
    setPreview(null)
    await refreshDetail()
  }
    ...
    <ProtectedRoute loading={!state && !error} error={error}>
      <div className="space-y-6">
        <header className="flex flex-wrap items-start justify-between gap-4">
          <div><p className="text-sm font-semibold uppercase text-blue-700">Offer Preparation</p><h1 className="mt-2 text-2xl font-semibold text-slate-950">Itinerary Builder</h1><p className="mt-1 max-w-4xl text-sm text-slate-600">Build accurate itinerary drafts from pasted, parser, booking-import, existing itinerary, or manual data. Raw source text is preserved; unknown values stay unknown; application is explicit.</p></div>
        ...
        <div className="grid gap-6 xl:grid-cols-[260px_minmax(0,1fr)]">
          ...
          {session ? <main className="min-w-0 space-y-7">
            <section className="border-b border-slate-200 pb-5"><div className="flex flex-wrap items-start justify-between gap-4"><div><p className="text-xs font-semibold uppercase text-slate-500">{title(session.authoring_mode)} session</p><h2 className="mt-1 text-xl font-semibold text-slate-950">{session.title}</h2><p className="mt-2 text-sm text-slate-600">Last saved {dateTime(session.updated_at)}{session.journey_id ? " · Linked itinerary ready" : " · Not yet linked to an itinerary"}</p></div><Status value={session.status} /></div><div className="mt-4 grid gap-3 sm:grid-cols-4"><Metric label="Completeness" value={`${session.completeness_score || 0}%`} /><Metric label="Segments" value={activeSegments.length} /><Metric label="Blocking" value={session.blocking_errors_count || 0} danger /><Metric label="Warnings" value={session.warnings_count || 0} warning /></div></section>
            ...
            <section className="border-t border-slate-200 pt-5"><div className="flex items-center gap-2"><GitBranch className="h-5 w-5 text-blue-700" /><h3 className="text-lg font-semibold text-slate-950">Apply to itinerary</h3></div><p className="mt-1 text-sm text-slate-600">Preview first. Applying creates canonical itinerary records with source links; finalized snapshots are immutable and no client snapshot is published.</p><div className="mt-4 grid gap-3 md:grid-cols-3"><Select value={applySettings.application_mode} onChange={(value) => setApplySettings({ ...applySettings, application_mode: value })} options={["create_new_journey", "create_new_option", "append_to_option", "replace_draft_option", "update_unfinalized_option"]} /><input value={applySettings.journey_id} onChange={(event) => setApplySettings({ ...applySettings, journey_id: event.target.value })} placeholder="Existing itinerary reference when required" className="field" /><input value={applySettings.option_id} onChange={(event) => setApplySettings({ ...applySettings, option_id: event.target.value })} placeholder="Option ID when required" className="field" /></div><div className="mt-3 flex flex-wrap gap-2"><button type="button" onClick={() => previewApplication().catch(fail(setError))} className="secondary-button"><Eye className="h-4 w-4" />Preview itinerary</button>{preview && !preview.blocking ? <button type="button" onClick={() => applyJourney().catch(fail(setError))} className="primary-button"><GitBranch className="h-4 w-4" />Apply to itinerary</button> : null}{detail?.session?.journey_id ? <><a href={`/agency/journey-option-composition?journey_id=${encodeURIComponent(detail.session.journey_id)}&authoring_session_id=${encodeURIComponent(detail.session.id)}`} className="secondary-button"><Sparkles className="h-4 w-4" />Compose itinerary options</a><a href={`/agency/journey-comparison-presentations?journey_id=${encodeURIComponent(detail.session.journey_id)}`} className="secondary-button"><Sparkles className="h-4 w-4" />Prepare Offer Comparison</a></> : null}</div>{preview ? <div className={`mt-4 rounded-md border p-4 ${preview.blocking ? "border-red-200 bg-red-50" : "border-emerald-200 bg-emerald-50"}`}><p className="font-semibold text-slate-950">{preview.blocking ? "Application blocked" : "Ready for explicit application"}</p><p className="mt-1 text-sm text-slate-700">{preview.segment_count} segments · {preview.connections?.length || 0} calculated connections · {preview.validation_summary?.blocking || 0} blockers. {preview.target_reason || "Source references will be retained."}</p></div> : null}</section>
