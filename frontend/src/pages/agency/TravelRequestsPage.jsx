...
export default function TravelRequestsPage() {
  ...
  async function load(nextFilters = filters) {
    ...
    const [requests, summary] = await Promise.all([
      apiGet(`/api/agencies/${context.agency.id}/travel-request-workspaces${query}`),
      apiGet(`/api/agencies/${context.agency.id}/travel-request-workspaces/summary`),
    ])
    setState({
      ...context,
      requests: requests.items || [],
      summary: requests.summary || summary.summary || {},
    })
  }

  useEffect(() => {
    load(filters).catch((err) => setError(err.message))
  }, [filters.status, filters.request_type, filters.priority, filters.assigned_agent, filters.departure_date])
        ...
        <div className="space-y-6">
          <div className="flex flex-wrap items-start justify-between gap-3">
            <div>
              ...
              <h2 className="mt-2 text-2xl font-semibold text-slate-950">Travel Requests</h2>
              <p className="mt-1 text-sm text-slate-600">Read-only travel request workspace metadata. These records do not execute bookings, issue tickets, connect to live GDS or NDC, process payments, send email or SMS, run AI automation, call external APIs, integrate suppliers, call live airlines, run background workers, automatically convert requests to trips, or automatically create offers.</p>
            </div>
            <div className="flex flex-wrap gap-2">
              ...
              <span className="rounded-full bg-slate-100 px-3 py-2 text-xs font-semibold text-slate-700">No automatic conversion</span>
            </div>
          </div>
          ...
          {state?.requests?.length ? <RequestList requests={state.requests} /> : <EmptyState title="No travel requests" body="Travel request workspace metadata will appear here after platform records are created for this agency." />}
        </div>
      </ProtectedRoute>
    </AgencyLayout>
  )
