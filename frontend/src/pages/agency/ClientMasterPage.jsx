...
export default function ClientMasterPage() {
  ...
  async function load(nextFilters = filters) {
    ...
    const response = await apiGet(`/api/agencies/${context.agency.id}/client-master${query}`)
    setState({ ...context, items: response.items || response.clients || [], summary: response.summary || {} })
  }

  useEffect(() => {
    load(filters).catch((err) => setError(err.message))
  }, [filters.search, filters.status, filters.portal_status, filters.passenger])
