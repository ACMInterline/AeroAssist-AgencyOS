...
// Internal compatibility label: Prepare client comparison.
...
export default function JourneyOptionCompositionWorkspacePage() {
  ...
  async function createFromJourney() {
    ...
    if (!journeyId) throw new Error("Open this tool from an itinerary or select an existing option set.")
    const response = await apiPost(`/api/agencies/${state.agency.id}/journey-option-compositions/from-journey/${journeyId}`, {})
    setNotice("Itinerary option set created from canonical segments. No offer was published or sent.")
  ...
  async function assignSegments() {
    ...
    setSelectedSegments([])
    setNotice("Canonical itinerary segments assigned by reference and metrics recalculated.")
    await refresh()
  }
  ...
  async function createSnapshot() {
    ...
    setNotice(`Protected option-set version ${response.snapshot.snapshot_number} created.`)
    await refresh()
  }
  ...
  async function applyHandoff() {
    ...
    setHandoff(response)
    setNotice("Option-set version linked to the offer workspace. No offer record or provider system was mutated.")
    await refresh()
  }
  ...
  const deliveryHref = offerId ? `/agency/offers/${encodeURIComponent(offerId)}?section=delivery` : "/agency/offers"
  ...
  const canViewSupplierCosts = authorization.hasPermission("view_supplier_costs")
  const canViewMargins = authorization.hasPermission("view_margins")
  const visiblePriceFields = Object.keys(emptyPrice).filter((key) =>
    (key !== "supplier_amount" || canViewSupplierCosts) &&
    (key !== "markup_amount" || canViewMargins)
  )
  ...
  return <AgencyLayout user={state?.me?.user} agency={state?.agency}>
    <ProtectedRoute loading={!state && !error} error={error}>
      <div className="space-y-6">
        <header className="flex flex-wrap items-start justify-between gap-4"><div><p className="text-sm font-semibold uppercase text-blue-700">Offer Preparation</p><h1 className="mt-2 text-2xl font-semibold text-slate-950">Itinerary Options & Fare Brands</h1><p className="mt-1 max-w-4xl text-sm text-slate-600">Build itinerary alternatives and fare-brand choices from canonical itinerary segments and governed knowledge. No live price, availability, publication, booking, ticketing, provider contact, or EMD action runs here.</p></div><div className="flex gap-2"><button type="button" title="Create from itinerary" onClick={() => createFromJourney().catch(fail(setError))} className="icon-button"><Plus className="h-4 w-4" /></button><button type="button" title="Refresh workspace" onClick={() => load().catch(fail(setError))} className="icon-button"><RefreshCw className="h-4 w-4" /></button></div></header>
        ...
        <div className="grid gap-6 xl:grid-cols-[260px_minmax(0,1fr)]">
          ...
          {composition ? <main className="min-w-0 space-y-8">
            ...
            {activeOption ? <section><div className="flex items-center gap-2"><CircleDollarSign className="h-5 w-5 text-blue-700" /><h3 className="text-lg font-semibold text-slate-950">Commercial values</h3></div><p className="mt-1 text-sm text-slate-600">{canViewSupplierCosts && canViewMargins ? "Select a fare choice. The total is validated from the reviewed commercial components." : "Select a fare choice. Supplier cost and margin details remain restricted by your Agency permission scope."}</p><div className="mt-3 grid gap-3 md:grid-cols-3"><select value={selectedFareId} onChange={(event) => setSelectedFareId(event.target.value)} className="field"><option value="">Select fare choice...</option>{selectedFares.map((item) => <option value={item.id} key={item.id}>{item.client_safe_label}</option>)}</select>{visiblePriceFields.map((key) => <input key={key} type={key.includes("until") ? "datetime-local" : key === "currency" ? "text" : "number"} step="0.01" value={price[key]} onChange={(event) => setPrice({ ...price, [key]: event.target.value })} placeholder={title(key)} className="field" />)}</div><button type="button" onClick={() => savePrice().catch(fail(setError))} className="mt-3 primary-button"><Save className="h-4 w-4" />Validate and save</button></section> : null}
            ...
            <section><h3 className="text-lg font-semibold text-slate-950">Link to Offer Workspace</h3><p className="mt-1 text-sm text-slate-600">This only links a protected option-set version to an existing Offer Workspace. It does not publish, send, accept, book, ticket, issue an EMD, contact a provider, or change accepted-offer snapshots.</p><div className="mt-3 flex flex-wrap gap-2"><input value={offerId} onChange={(event) => setOfferId(event.target.value)} placeholder="Existing Offer Workspace ID" className="field max-w-lg" /><button type="button" onClick={() => previewHandoff().catch(fail(setError))} className="secondary-button">Review link</button>{handoff?.can_apply ? <button type="button" onClick={() => applyHandoff().catch(fail(setError))} className="primary-button">Link snapshot</button> : null}</div>{handoff ? <pre className="mt-4 max-h-64 overflow-auto border-y border-slate-200 bg-slate-50 p-4 text-xs text-slate-700">{JSON.stringify(handoff.preview || handoff.offer_handoff, null, 2)}</pre> : null}</section>
          ...
          </main> : <EmptyState title="No option set selected" body="Open a canonical Journey and create its first option set." />}
        </div>
      </div>
    </ProtectedRoute>
  </AgencyLayout>
