...
export default function RolloutRollbackPlansPage() {
  ...
  async function load(nextFilters = filters) {
    ...
    const [rollbackPlans, summary] = await Promise.all([
      apiGet(`/api/agencies/${context.agency.id}/feature-bundle-rollout-rollback-plans${query}`),
      apiGet(`/api/agencies/${context.agency.id}/feature-bundle-rollout-rollback-plans/summary`),
    ])
    setState({
      ...context,
      rollbackPlans: rollbackPlans.items || [],
      summary: rollbackPlans.summary || summary.summary || {},
    })
  }
        ...
        <div className="space-y-6">
          <div className="flex flex-wrap items-start justify-between gap-3">
            <div>
              ...
              <h2 className="mt-2 text-2xl font-semibold text-slate-950">Rollout Rollback Plans</h2>
              <p className="mt-1 text-sm text-slate-600">Read-only rollout rollback plan metadata. These records do not execute rollbacks, automate deployments, activate or deactivate features, enforce entitlements, bill, call providers or external APIs, use AI, notify users, publish, send email, execute webhooks, or switch runtime behavior.</p>
            </div>
          ...
          {state?.rollbackPlans?.length ? <RollbackPlanList rollbackPlans={state.rollbackPlans} /> : <EmptyState title="No rollout rollback plans" body="Platform rollback plan metadata will appear here after records are created." />}
        </div>
      </ProtectedRoute>
    </AgencyLayout>
  )
        ...
        <tbody className="divide-y divide-slate-100">
          {rollbackPlans.map((rollbackPlan) => (
            <tr key={rollbackPlan.id}>
              ...
              <td className="px-4 py-3 align-top text-slate-700">{rollbackPlan.plan_name || rollbackPlan.rollout_plan_id}</td>
