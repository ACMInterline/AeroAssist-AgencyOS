...
export default function FlightWorkspacesPage() {
  ...
  async function load(nextFilters = filters) {
    ...
    const [flights, summary] = await Promise.all([
      apiGet(`/api/agencies/${context.agency.id}/flight-workspaces${query}`),
      apiGet(`/api/agencies/${context.agency.id}/flight-workspaces/summary`),
    ])
    setState({
      ...context,
      flights: flights.items || [],
      summary: flights.summary || summary.summary || {},
    })
  }

  useEffect(() => {
    load(filters).catch((err) => setError(err.message))
  }, [filters.status, filters.airline, filters.departure_airport, filters.arrival_airport, filters.departure_date, filters.cabin, filters.booking_class])
        ...
        <div className="space-y-6">
          <div className="flex flex-wrap items-start justify-between gap-3">
            <div>
              ...
              <h2 className="mt-2 text-2xl font-semibold text-slate-950">Flights</h2>
              <p className="mt-1 text-sm text-slate-600">Read-only flight workspace metadata. These records do not execute bookings, run live flight search, connect to GDS or NDC, call airline APIs, process payments, issue tickets, synchronize schedules, call external APIs, use AI, run background workers, automatically generate routes, validate flights, look up airlines, or update live schedules.</p>
            </div>
            <div className="flex flex-wrap gap-2">
              ...
              <span className="rounded-full bg-slate-100 px-3 py-2 text-xs font-semibold text-slate-700">No schedule sync</span>
            </div>
          </div>
          ...
          {state?.flights?.length ? <FlightList flights={state.flights} /> : <EmptyState title="No flight workspaces" body="Flight workspace metadata will appear here after platform records are created for this agency." />}
        </div>
      </ProtectedRoute>
    </AgencyLayout>
  )
