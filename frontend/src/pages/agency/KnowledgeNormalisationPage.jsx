...
export default function KnowledgeNormalisationPage() {
  ...
  async function load(nextFilters = filters) {
    ...
    const response = await apiGet(`/api/agencies/${context.agency.id}/airline-knowledge-normalisation${query}`)
    setState({ ...context, normalisations: response.items || [], summary: response.summary || {} })
  }

  useEffect(() => {
    load(filters).catch((err) => setError(err.message))
  }, [filters.normalisation_status, filters.normalisation_type, filters.canonical_code, filters.taxonomy_domain, filters.taxonomy_family, filters.taxonomy_variant, filters.airline, filters.ssr_code, filters.rfic, filters.rfisc, filters.review_status, filters.approval_status])
        ...
        <div className="space-y-6">
          <div className="flex flex-wrap items-start justify-between gap-3">
            <div>
              ...
              <h2 className="mt-2 text-2xl font-semibold text-slate-950">Knowledge Normalisation</h2>
              <p className="mt-1 text-sm text-slate-600">Read-only canonical vocabulary metadata for airline operations. These records normalize airline, GDS, commercial, and operational terms for consistent future AOIE comparison without live evaluation, AI parsing, recommendations, feasibility scoring, pricing calculation, scraping, workers, or providers.</p>
            </div>
            <div className="flex flex-wrap gap-2">
              <span className="rounded-full bg-blue-50 px-3 py-2 text-xs font-semibold text-blue-700">Read-only</span>
              <span className="rounded-full bg-slate-100 px-3 py-2 text-xs font-semibold text-slate-700">Canonical vocabulary</span>
            </div>
          </div>
          ...
          {state?.normalisations?.length ? <NormalisationList normalisations={state.normalisations} /> : <EmptyState title="No knowledge normalisations" body="Canonical vocabulary metadata shared with this agency will appear here." />}
        </div>
      </ProtectedRoute>
    </AgencyLayout>
  )
...
function NormalisationList({ normalisations }) {
  return (
    <section className="space-y-3">
      {normalisations.map((item) => (
        <details key={item.id} className="rounded-lg border border-slate-200 bg-white p-4" open>
          <summary className="cursor-pointer list-none">
            <div className="grid gap-3 md:grid-cols-[1fr_180px_180px]">
              ...
              <div className="text-xs text-slate-600">
                <p>Status: {formatType(item.normalisation_status)}</p>
          ...
          <NormalisationSections item={item} />
        </details>
      ))}
    </section>
  )
...
function NormalisationSections({ item }) {
  return (
    <div className="mt-4 space-y-3 text-xs text-slate-600">
      ...
      <Section title="Aliases / Terms"><RecordPanel value={terms(item)} /></Section>
      <Section title="Applicability"><RecordPanel value={applicability(item)} /></Section>
      <Section title="Animal Taxonomy"><RecordPanel value={animalTaxonomy(item)} /></Section>
      <Section title="Aircraft / Cabin Taxonomy"><RecordPanel value={aircraftCabinTaxonomy(item)} /></Section>
      <Section title="Service Taxonomy"><RecordPanel value={serviceTaxonomy(item)} /></Section>
      <Section title="Units"><RecordPanel value={units(item)} /></Section>
      <Section title="Knowledge Links"><RecordPanel value={knowledgeLinks(item)} /></Section>
...
function terms(item) {
  return { aliases: item.aliases, abbreviations: item.abbreviations, airline_specific_terms: item.airline_specific_terms, gds_terms: item.gds_terms, commercial_terms: item.commercial_terms, operational_terms: item.operational_terms }
}

function applicability(item) {
  return { airline_codes: item.airline_codes, country_codes: item.country_codes, airport_codes: item.airport_codes, aircraft_types: item.aircraft_types, cabin_codes: item.cabin_codes, service_codes: item.service_codes, ssr_codes: item.ssr_codes, rfic_codes: item.rfic_codes, rfisc_codes: item.rfisc_codes }
}

function animalTaxonomy(item) {
  return { species: item.species, breed: item.breed, breed_group: item.breed_group, brachycephalic_flag: item.brachycephalic_flag, restricted_breed_flag: item.restricted_breed_flag, service_animal_flag: item.service_animal_flag, emotional_support_animal_flag: item.emotional_support_animal_flag, animal_notes: item.animal_notes }
}

function aircraftCabinTaxonomy(item) {
  return { aircraft_family: item.aircraft_family, aircraft_subtype: item.aircraft_subtype, cabin_family: item.cabin_family, cabin_name: item.cabin_name, seat_type: item.seat_type, fixed_armrest_flag: item.fixed_armrest_flag, adjacent_seat_relevance: item.adjacent_seat_relevance, under_seat_space_relevance: item.under_seat_space_relevance, cabin_notes: item.cabin_notes }
}

function serviceTaxonomy(item) {
  return { passenger_need_category: item.passenger_need_category, service_domain: item.service_domain, service_family: item.service_family, service_variant: item.service_variant, related_ssr_code: item.related_ssr_code, related_osi_relevance: item.related_osi_relevance, related_emd_relevance: item.related_emd_relevance, related_document_relevance: item.related_document_relevance }
}

function units(item) {
  return { unit_type: item.unit_type, canonical_unit: item.canonical_unit, unit_aliases: item.unit_aliases, conversion_notes: item.conversion_notes }
}

function knowledgeLinks(item) {
  return { acquisition_ids: item.acquisition_ids, constraint_ids: item.constraint_ids, evidence_reference_ids: item.evidence_reference_ids, policy_reference_ids: item.policy_reference_ids, pricing_reference_ids: item.pricing_reference_ids, capability_reference_ids: item.capability_reference_ids }
}
