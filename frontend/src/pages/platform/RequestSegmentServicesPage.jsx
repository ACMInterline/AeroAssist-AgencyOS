...
export default function RequestSegmentServicesPage() {
  ...
  async function load(nextFilters = filters) {
    ...
    const [me, agencies, response] = await Promise.all([
      apiGet("/api/auth/me"),
      apiGet("/api/agencies"),
      apiGet(`/api/platform/request-segment-services${query}`),
    ])
    setState({
      me,
      agencies: agencies.items || [],
      items: response.items || response.scopes || [],
  ...
  }, [filters.agency_id, filters.request, filters.passenger, filters.segment, filters.service_family, filters.ssr_code, filters.pet_transport_mode, filters.item_category, filters.readiness_status, filters.requires_policy_review, filters.requires_document_followup])
        ...
        <div className="space-y-6">
          <div className="flex flex-wrap items-start justify-between gap-3">
            <div>
              ...
              <p className="mt-1 text-sm text-slate-600">Metadata-only passenger + segment + service scope records for segment-first intake. Requests stay intake records; trips stay operational dossiers.</p>
            </div>
            <div className="flex flex-wrap gap-2">
              <span className="rounded-full bg-blue-50 px-3 py-2 text-xs font-semibold text-blue-700">Segment first</span>
              <span className="rounded-full bg-slate-100 px-3 py-2 text-xs font-semibold text-slate-700">No evaluation</span>
            </div>
          </div>
          ...
          <section className="space-y-3">
            ...
            {items.length ? <ScopeList items={items} showAgency /> : <EmptyState title="No request segment services" body="Segment-first passenger service scope metadata will appear here." />}
          </section>
        </div>
      </ProtectedRoute>
    </PlatformLayout>
...
function ScopeList({ items, showAgency = false }) {
  return (
          ...
          <summary className="cursor-pointer list-none">
            <div className="grid gap-3 md:grid-cols-[1fr_190px_190px]">
              ...
              <div className="text-xs text-slate-600">
                ...
                <p className="mt-1">Request: {item.travel_request_id || item.request_reference || "Unset"}</p>
              </div>
              <div className="text-xs text-slate-600">
