...
const base = "/api/platform/offer-decision-export-audit-reviews"
...
export default function OfferDecisionExportAuditReviewsPage() {
  ...
  async function load() {
    const [me, summary, reviews, findings, checklist, snapshots] = await Promise.all([
      apiGet("/api/auth/me"),
      apiGet(`${base}/summary`),
      apiGet(`${base}/reviews`),
      apiGet(`${base}/findings`),
      apiGet(`${base}/checklist-items`),
        ...
        <div className="space-y-6">
          <div className="flex flex-wrap items-start justify-between gap-3">
            <div>
              ...
              <h2 className="mt-2 text-2xl font-semibold text-slate-950">Audit Review Diagnostics</h2>
              <p className="mt-1 text-sm text-slate-600">Read-only visibility into lifecycle audit review metadata, findings, checklist items, and snapshots.</p>
            </div>
          ...
          <section className="grid gap-4 xl:grid-cols-2">
            <SimpleList title="Reviews" items={state?.reviews || []} fields={["agency_id", "title", "review_status", "completion_score"]} />
            <SimpleList title="Findings" items={state?.findings || []} fields={["agency_id", "finding_type", "severity", "finding_status"]} />
            <SimpleList title="Checklist" items={state?.checklist || []} fields={["agency_id", "item_key", "item_status", "required"]} />
