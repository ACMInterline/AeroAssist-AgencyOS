...
export default function OperationalRuleComposerPage() {
  ...
  async function load(nextFilters = filters) {
    ...
    const [me, agencies, response] = await Promise.all([
      apiGet("/api/auth/me"),
      apiGet("/api/agencies"),
      apiGet(`/api/platform/operational-rule-composer${query}`),
    ])
    setState({
      me,
      agencies: agencies.items || [],
      items: response.items || response.rules || [],
  ...
  }, [filters.agency_id, filters.rule_family, filters.service_family, filters.service_code, filters.lifecycle_status, filters.severity, filters.operator, filters.search])
        ...
        <div className="space-y-6">
          <div className="flex flex-wrap items-start justify-between gap-3">
            <div>
              ...
              <h2 className="mt-2 text-2xl font-semibold text-slate-950">Operational Rule Composer</h2>
              <p className="mt-1 text-sm text-slate-600">No-code compound rule metadata for airline passenger service restrictions and outcomes.</p>
            </div>
          ...
          <section className="space-y-3">
            ...
            {items.length ? <RuleList items={items} showAgency /> : <EmptyState title="No operational rules" body="Operational Rule Composer metadata will appear here." />}
          </section>
        </div>
      </ProtectedRoute>
    </PlatformLayout>
...
function RuleList({ items, showAgency = false }) {
  return (
          ...
          <summary className="cursor-pointer list-none">
            <div className="grid gap-3 md:grid-cols-[1fr_190px_190px]">
              ...
              <div className="text-xs text-slate-600">
                ...
                <p className="mt-1">Family: {formatType(item.rule_family)}</p>
              </div>
              <div className="text-xs text-slate-600">
