...
export default function PlatformTripWorkspacesPage() {
  ...
  async function load(nextFilters = filters) {
    ...
    const [me, agencies, trips] = await Promise.all([
      apiGet("/api/auth/me"),
      apiGet("/api/agencies"),
      apiGet(`/api/platform/trip-workspaces${query}`),
    ])
    setState({
      me,
  ...
  }, [filters.agency_id, filters.status, filters.departure_country, filters.destination_country, filters.departure_date, filters.assigned_agent, filters.priority])
        ...
        <div className="space-y-6">
          <div className="flex flex-wrap items-start justify-between gap-3">
            <div>
              ...
              <h2 className="mt-2 text-2xl font-semibold text-slate-950">Trip Workspaces</h2>
              <p className="mt-1 text-sm text-slate-600">Metadata-only trip workspace records. These records do not execute bookings, issue tickets, connect to GDS or NDC, call airline APIs, process payments, create invoices, use AI, run background workers, automatically generate trips, automatically generate itineraries, call external integrations, or automate journeys.</p>
            </div>
          ...
          {state?.trips?.length ? <TripList trips={state.trips} showAgency /> : <EmptyState title="No trip workspaces" body="Trip workspace metadata will appear here after platform records are created." />}
        </div>
      </ProtectedRoute>
    </PlatformLayout>
  )
...
function TripList({ trips, showAgency = false }) {
  return (
        ...
        <tbody className="divide-y divide-slate-100">
          {trips.map((trip) => (
            <tr key={trip.id}>
              ...
              {showAgency ? <td className="px-4 py-3 align-top text-slate-700">{trip.agency_name || trip.agency_id}</td> : null}
