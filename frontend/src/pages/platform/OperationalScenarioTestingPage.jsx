...
export default function OperationalScenarioTestingPage() {
  ...
  async function load(nextFilters = filters) {
    ...
    const [me, agencies, response] = await Promise.all([
      apiGet("/api/auth/me"),
      apiGet("/api/agencies"),
      apiGet(`/api/platform/operational-scenario-testing${query}`),
    ])
    setState({
      me,
      agencies: agencies.items || [],
      items: response.items || response.scenarios || [],
  ...
  }, [filters.agency_id, filters.scenario_family, filters.test_status, filters.airline_code, filters.service_code, filters.expected_recommendation_level, filters.search])
        ...
        <div className="space-y-6">
          <div className="flex flex-wrap items-start justify-between gap-3">
            <div>
              ...
              <p className="mt-1 text-sm text-slate-600">Metadata-only passenger service scenario test cases for validating knowledge production. No live providers, AI, parser execution, workers, or automated evaluation.</p>
            </div>
            <div className="flex flex-wrap gap-2">
              <span className="rounded-full bg-blue-50 px-3 py-2 text-xs font-semibold text-blue-700">Metadata only</span>
              <span className="rounded-full bg-slate-100 px-3 py-2 text-xs font-semibold text-slate-700">No live testing</span>
            </div>
          </div>
          ...
          <section className="space-y-3">
            ...
            {items.length ? <ScenarioList items={items} showAgency /> : <EmptyState title="No scenario tests" body="Passenger service scenario test metadata will appear here." />}
          </section>
        </div>
      </ProtectedRoute>
    </PlatformLayout>
...
function ScenarioList({ items, showAgency = false }) {
  return (
          ...
          <summary className="cursor-pointer list-none">
            <div className="grid gap-3 md:grid-cols-[1fr_190px_190px]">
              ...
              <div className="text-xs text-slate-600">
                ...
                <p className="mt-1">Family: {formatType(item.scenario_family)}</p>
              </div>
              <div className="text-xs text-slate-600">
