...
export default function OperationalEvaluationsPage() {
  ...
  async function load(nextFilters = filters) {
    ...
    const [me, agencies, response] = await Promise.all([
      apiGet("/api/auth/me"),
      apiGet("/api/agencies"),
      apiGet(`/api/platform/operational-evaluations${query}`),
    ])
    setState({
      me,
      agencies: agencies.items || [],
      items: response.items || response.evaluations || [],
        ...
        <div className="space-y-6">
          <div className="flex flex-wrap items-start justify-between gap-3">
            <div>
              ...
              <h2 className="mt-2 text-2xl font-semibold text-slate-950">Operational Knowledge Evaluations</h2>
              <p className="mt-1 text-sm text-slate-600">Metadata-only deterministic evaluation records for what operational knowledge applies. Evaluation is not recommendation and does not determine passenger feasibility, use AI or LLM prompts, search flights, book, ticket, call providers, execute parsers, optimise pricing, or run workers.</p>
            </div>
            <div className="flex flex-wrap gap-2">
              <span className="rounded-full bg-blue-50 px-3 py-2 text-xs font-semibold text-blue-700">Structured explanations</span>
          ...
          <section className="space-y-3">
            ...
            {items.length ? <EvaluationList items={items} showAgency /> : <EmptyState title="No operational evaluations" body="Operational knowledge evaluation metadata will appear here." />}
          </section>
        </div>
      </ProtectedRoute>
    </PlatformLayout>
...
function EvaluationList({ items, showAgency = false }) {
  return (
          ...
          <summary className="cursor-pointer list-none">
            <div className="grid gap-3 md:grid-cols-[1fr_190px_190px]">
              ...
              <div className="text-xs text-slate-600">
                ...
                <p className="mt-1">Status: {formatType(item.evaluation_status)}</p>
              </div>
              <div className="text-xs text-slate-600">
                <p>Risk: {formatType(item.operational_risk)}</p>
                <p className="mt-1">Completed: {item.evaluation_completed ? "Yes" : "No"}</p>
              </div>
            </div>
          </summary>
