...
export default function PricingFormulaBuilderPage() {
  ...
  async function load(nextFilters = filters) {
    ...
    const [me, agencies, response] = await Promise.all([
      apiGet("/api/auth/me"),
      apiGet("/api/agencies"),
      apiGet(`/api/platform/pricing-formula-builder${query}`),
    ])
    setState({
      me,
      agencies: agencies.items || [],
      items: response.items || response.pricing_formulas || [],
        ...
        <div className="space-y-6">
          <div className="flex flex-wrap items-start justify-between gap-3">
            <div>
              ...
              <p className="mt-1 text-sm text-slate-600">No-code metadata for airline ancillary and service pricing formulas, visibility, and refund or exchange conditions.</p>
            </div>
          ...
          <section className="space-y-3">
            ...
            {items.length ? <FormulaList items={items} showAgency /> : <EmptyState title="No pricing formulas" body="Pricing Formula Builder metadata will appear here." />}
          </section>
        </div>
      </ProtectedRoute>
    </PlatformLayout>
...
function FormulaList({ items, showAgency = false }) {
  return (
          ...
          <summary className="cursor-pointer list-none">
            <div className="grid gap-3 md:grid-cols-[1fr_190px_190px]">
              ...
              <div className="text-xs text-slate-600">
                ...
                <p className="mt-1">Category: {formatType(item.pricing_category)}</p>
              </div>
              <div className="text-xs text-slate-600">
                <p>Amount: {formatType(item.amount_type)}</p>
                <p className="mt-1">Visibility: {formatType(item.client_visibility)}</p>
              </div>
            </div>
          </summary>
