...
export default function PassengerMasterPage() {
  ...
  async function load(nextFilters = filters) {
    ...
    const [me, agencies, response] = await Promise.all([
      apiGet("/api/auth/me"),
      apiGet("/api/agencies"),
      apiGet(`/api/platform/passenger-master${query}`),
    ])
    setState({ me, agencies: agencies.items || [], items: response.items || response.passengers || [], summary: response.summary || {} })
  }

  useEffect(() => {
    load(filters).catch((err) => setError(err.message))
  }, [filters.agency_id, filters.search, filters.status, filters.service])
        ...
        <div className="space-y-6">
          <div className="flex flex-wrap items-start justify-between gap-3">
            <div>
              ...
              <h2 className="mt-2 text-2xl font-semibold text-slate-950">Passenger Master</h2>
              <p className="mt-1 text-sm text-slate-600">Reusable operational identity metadata across requests, trips, booking mirrors, ticket mirrors, EMD mirrors, documents, evaluations, feasibility, and recommendations.</p>
            </div>
          ...
          <section className="space-y-3">
            <div className="flex items-center justify-between gap-3">
              <h3 className="text-lg font-semibold text-slate-950">Passengers</h3>
              <span className="rounded-full bg-slate-100 px-3 py-1 text-xs font-semibold text-slate-600">{items.length}</span>
            </div>
            {items.length ? <MasterRecordList items={items} type="passenger" showAgency /> : <EmptyState title="No passenger master records" body="Passenger operational identity metadata will appear here." />}
          </section>
        </div>
      </ProtectedRoute>
    </PlatformLayout>
