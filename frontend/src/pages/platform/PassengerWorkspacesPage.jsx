...
export default function PlatformPassengerWorkspacesPage() {
  ...
  async function load(nextFilters = filters) {
    ...
    const [me, agencies, passengers] = await Promise.all([
      apiGet("/api/auth/me"),
      apiGet("/api/agencies"),
      apiGet(`/api/platform/passenger-workspaces${query}`),
    ])
    setState({
      me,
  ...
  }, [filters.agency_id, filters.status, filters.nationality, filters.citizenship, filters.assistance_profile, filters.travel_date, filters.operational_workspace_id])
        ...
        <div className="space-y-6">
          <div className="flex flex-wrap items-start justify-between gap-3">
            <div>
              ...
              <h2 className="mt-2 text-2xl font-semibold text-slate-950">Passenger Workspaces</h2>
              <p className="mt-1 text-sm text-slate-600">Metadata-only passenger workspace records. These records do not execute bookings, issue tickets, connect to GDS or NDC, process payments, integrate suppliers, use AI, send email or SMS, run background workers, call external APIs, automatically match profiles, automatically validate documents, or communicate with airlines.</p>
            </div>
            <div className="flex flex-wrap gap-2">
              ...
              <span className="rounded-full bg-slate-100 px-3 py-2 text-xs font-semibold text-slate-700">No matching or validation</span>
            </div>
          </div>
          ...
          {state?.passengers?.length ? <PassengerList passengers={state.passengers} /> : <EmptyState title="No passenger workspaces" body="Passenger workspace metadata will appear here after platform records are created." />}
        </div>
      </ProtectedRoute>
    </PlatformLayout>
  )
        ...
        <tbody className="divide-y divide-slate-100">
          {passengers.map((passenger) => (
            <tr key={passenger.id}>
              ...
              <td className="px-4 py-3 align-top text-slate-700">{passenger.agency_name || passenger.agency_id}</td>
