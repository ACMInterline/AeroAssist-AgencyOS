...
export default function KnowledgeQualityAssurancePage() {
  ...
  async function load(nextFilters = filters) {
    ...
    const [me, agencies, response] = await Promise.all([
      apiGet("/api/auth/me"),
      apiGet("/api/agencies"),
      apiGet(`/api/platform/knowledge-quality-assurance${query}`),
    ])
    setState({
      me,
  ...
  }, [filters.agency_id, filters.target_type, filters.airline_code, filters.service_family, filters.service_code, filters.qa_status, filters.severity, filters.issue_check, filters.approval_recommendation, filters.search])
        ...
        <div className="space-y-6">
          <div className="flex flex-wrap items-start justify-between gap-3">
            <div>
              ...
              <p className="mt-1 text-sm text-slate-600">Metadata-only QA reviews for airline knowledge production.</p>
            </div>
            <div className="flex flex-wrap gap-2">
              <span className="rounded-full bg-blue-50 px-3 py-2 text-xs font-semibold text-blue-700">Metadata only</span>
              <span className="rounded-full bg-slate-100 px-3 py-2 text-xs font-semibold text-slate-700">No auto-approval</span>
            </div>
          </div>
          ...
          <section className="space-y-3">
            ...
            {items.length ? <ReviewList items={items} showAgency /> : <EmptyState title="No QA reviews" body="Knowledge QA metadata will appear here." />}
          </section>
        </div>
      </ProtectedRoute>
    </PlatformLayout>
...
function ReviewList({ items, showAgency = false }) {
  return (
          ...
          <summary className="cursor-pointer list-none">
            <div className="grid gap-3 md:grid-cols-[1fr_190px_190px]">
              ...
              <div className="text-xs text-slate-600">
                ...
                <p className="mt-1">Target: {formatType(item.target_type)}</p>
              </div>
              <div className="text-xs text-slate-600">
