...
export default function AirlineRecommendationsPage() {
  ...
  async function load(nextFilters = filters) {
    ...
    const [me, agencies, response] = await Promise.all([
      apiGet("/api/auth/me"),
      apiGet("/api/agencies"),
      apiGet(`/api/platform/airline-recommendations${query}`),
    ])
    setState({
      me,
      agencies: agencies.items || [],
      items: response.items || response.recommendations || [],
  ...
  }, [filters.agency_id, filters.recommendation_status, filters.airline, filters.recommendation_level, filters.operational_score, filters.risk, filters.passenger_need_category, filters.cabin, filters.destination, filters.travel_date])
        ...
        <div className="space-y-6">
          <div className="flex flex-wrap items-start justify-between gap-3">
            <div>
              ...
              <h2 className="mt-2 text-2xl font-semibold text-slate-950">Airline Recommendations</h2>
              <p className="mt-1 text-sm text-slate-600">Metadata-only advisory recommendations. Recommendation is not feasibility, booking, provider search, live pricing, ticketing, EMD issuance, parser execution, AI generation, or final authority.</p>
            </div>
          ...
          <section className="space-y-3">
            ...
            {items.length ? <RecommendationList items={items} showAgency /> : <EmptyState title="No airline recommendations" body="Airline recommendation metadata will appear here." />}
          </section>
        </div>
      </ProtectedRoute>
    </PlatformLayout>
...
function RecommendationList({ items, showAgency = false }) {
  return (
          ...
          <summary className="cursor-pointer list-none">
            <div className="grid gap-3 md:grid-cols-[1fr_190px_190px]">
              ...
              <div className="text-xs text-slate-600">
                ...
                <p className="mt-1">Rank: {item.recommendation_rank ?? "Unset"}</p>
              </div>
              <div className="text-xs text-slate-600">
                <p>Status: {formatType(item.recommendation_status)}</p>
                <p className="mt-1">Ready: {item.recommendation_ready ? "Yes" : "No"}</p>
              </div>
            </div>
          </summary>
