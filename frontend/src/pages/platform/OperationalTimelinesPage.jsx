...
export default function PlatformOperationalTimelinesPage() {
  ...
  async function load(nextFilters = filters) {
    ...
    const [me, agencies, timelines] = await Promise.all([
      apiGet("/api/auth/me"),
      apiGet("/api/agencies"),
      apiGet(`/api/platform/operational-timelines${query}`),
    ])
    setState({
      me,
      agencies: agencies.items || [],
      entries: timelines.items || [],
  ...
  }, [filters.agency_id, filters.passenger, filters.booking, filters.ticket, filters.emd, filters.ssr, filters.airline, filters.communication_type, filters.event_type, filters.priority, filters.status, filters.date])
        ...
        <div className="space-y-6">
          <div className="flex flex-wrap items-start justify-between gap-3">
            <div>
              ...
              <h2 className="mt-2 text-2xl font-semibold text-slate-950">Operational Timelines</h2>
              <p className="mt-1 text-sm text-slate-600">Metadata-only operational history and communication records. Entries link passenger, request, trip, booking, ticket, EMD, SSR / OSI, and document workspaces without sending email, SMS, WhatsApp, Teams, Slack, airline messages, customer messages, AI summaries, workers, or provider integrations.</p>
            </div>
            <div className="flex flex-wrap gap-2">
              ...
              <span className="rounded-full bg-slate-100 px-3 py-2 text-xs font-semibold text-slate-700">No AI summaries</span>
            </div>
          </div>
          ...
          {state?.entries?.length ? <TimelineList entries={state.entries} showAgency /> : <EmptyState title="No timeline entries" body="Operational history metadata will appear here after platform records are created." />}
        </div>
      </ProtectedRoute>
    </PlatformLayout>
  )
...
function TimelineList({ entries, showAgency = false }) {
  return (
        ...
        <details key={entry.id} className="rounded-lg border border-slate-200 bg-white p-4" open>
          <summary className="cursor-pointer list-none">
            <div className="grid gap-3 md:grid-cols-[160px_1fr_180px]">
              ...
              <div>
                ...
                <p className="mt-1 text-sm text-slate-600">{formatType(entry.event_type)} · {entry.event_category || "uncategorized"} · {entry.event_status || "status unset"}</p>
              </div>
              <div className="text-xs text-slate-600">
                {showAgency ? <p>{entry.agency_name || entry.agency_id}</p> : null}
                <p className="mt-1">Priority: {entry.event_priority || "Unset"}</p>
              </div>
            </div>
          </summary>
          <div className="mt-4 grid gap-4 text-xs text-slate-600 lg:grid-cols-4">
            ...
            <DetailBlock title="Communication summary" lines={[
              `Type: ${entry.communication_type || "Unset"}`,
            ...
            <DetailBlock title="Notes and attachments" lines={[
              `Stage: ${entry.operational_stage || "Unset"}`,
              `Result: ${entry.operational_result || "Unset"}`,
              `Attachments: ${formatList(entry.attachment_ids)}`,
              `Internal only: ${entry.internal_only ? "Yes" : "No"}`,
