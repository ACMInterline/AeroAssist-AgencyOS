...
export default function PlatformOfferWorkspacesPage() {
  ...
  async function load(nextFilters = filters) {
    ...
    const [me, agencies, offers] = await Promise.all([
      apiGet("/api/auth/me"),
      apiGet("/api/agencies"),
      apiGet(`/api/platform/offer-workspaces${query}`),
    ])
    setState({
      me,
  ...
  }, [filters.agency_id, filters.status, filters.validity, filters.client_id, filters.destination, filters.min_price, filters.max_price, filters.assigned_agent])
        ...
        <div className="space-y-6">
          <div className="flex flex-wrap items-start justify-between gap-3">
            <div>
              ...
              <h2 className="mt-2 text-2xl font-semibold text-slate-950">Offer Workspaces</h2>
              <p className="mt-1 text-sm text-slate-600">Metadata-only offer workspace records. These records do not execute bookings, issue tickets, process payments, connect to GDS or NDC, call airline APIs, calculate fares, generate AI itineraries, integrate suppliers, call external APIs, automatically convert bookings, or run background workers.</p>
            </div>
            <div className="flex flex-wrap gap-2">
              ...
              <span className="rounded-full bg-slate-100 px-3 py-2 text-xs font-semibold text-slate-700">No fare calculations</span>
            </div>
          </div>
          ...
          {state?.offers?.length ? <OfferList offers={state.offers} showAgency /> : <EmptyState title="No offer workspaces" body="Offer workspace metadata will appear here after platform records are created." />}
        </div>
      </ProtectedRoute>
    </PlatformLayout>
  )
...
function OfferList({ offers, showAgency = false }) {
  return (
        ...
        <tbody className="divide-y divide-slate-100">
          {offers.map((offer) => (
            <tr key={offer.id}>
              ...
              {showAgency ? <td className="px-4 py-3 align-top text-slate-700">{offer.agency_name || offer.agency_id}</td> : null}
