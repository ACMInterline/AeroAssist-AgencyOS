...
export default function PlatformBookingWorkspacesPage() {
  ...
  async function load(nextFilters = filters) {
    ...
    const [me, agencies, bookings] = await Promise.all([
      apiGet("/api/auth/me"),
      apiGet("/api/agencies"),
      apiGet(`/api/platform/booking-workspaces${query}`),
    ])
    setState({
      me,
  ...
  }, [filters.agency_id, filters.status, filters.booking_owner, filters.airline, filters.supplier, filters.booking_date])
        ...
        <div className="space-y-6">
          <div className="flex flex-wrap items-start justify-between gap-3">
            <div>
              ...
              <h2 className="mt-2 text-2xl font-semibold text-slate-950">Booking Workspaces</h2>
              <p className="mt-1 text-sm text-slate-600">Metadata-only booking workspace records. These records do not create live bookings, issue tickets, connect to GDS or NDC, call airline APIs, process payments, calculate fares, use AI, run background workers, automatically confirm bookings, automatically generate tickets, or integrate external providers.</p>
            </div>
            <div className="flex flex-wrap gap-2">
              ...
              <span className="rounded-full bg-slate-100 px-3 py-2 text-xs font-semibold text-slate-700">No GDS or NDC</span>
            </div>
          </div>
          ...
          {state?.bookings?.length ? <BookingList bookings={state.bookings} showAgency /> : <EmptyState title="No booking workspaces" body="Booking workspace metadata will appear here after platform records are created." />}
        </div>
      </ProtectedRoute>
    </PlatformLayout>
  )
...
function BookingList({ bookings, showAgency = false }) {
  return (
        ...
        <tbody className="divide-y divide-slate-100">
          {bookings.map((booking) => (
            <tr key={booking.id}>
              ...
              {showAgency ? <td className="px-4 py-3 align-top text-slate-700">{booking.agency_name || booking.agency_id}</td> : null}
