...
export default function PlatformEmdWorkspacesPage() {
  ...
  async function load(nextFilters = filters) {
    ...
    const [me, agencies, emds] = await Promise.all([
      apiGet("/api/auth/me"),
      apiGet("/api/agencies"),
      apiGet(`/api/platform/emd-workspaces${query}`),
    ])
    setState({
      me,
      agencies: agencies.items || [],
      emds: emds.items || [],
      summary: emds.summary || {},
    })
  }

  useEffect(() => {
    load(filters).catch((err) => setError(err.message))
  }, [filters.agency_id, filters.status, filters.emd_type, filters.emd_a_or_s, filters.validating_carrier, filters.passenger, filters.rfic, filters.rfisc, filters.service_category, filters.issue_date])
  ...
  const metrics = [
    ...
    ["Coupons", state?.summary?.emd_coupon_detail_count || 0],
  ]
        ...
        <div className="space-y-6">
          <div className="flex flex-wrap items-start justify-between gap-3">
            <div>
              ...
              <h2 className="mt-2 text-2xl font-semibold text-slate-950">EMD Workspaces</h2>
              <p className="mt-1 text-sm text-slate-600">Metadata-only EMD workspace records. They link to ticket/EMD mirror data, service mechanics, RFIC/RFISC metadata, SSR/OSI references, and ancillary pricing notes without EMD issuance, exchange, refund, voiding, validation engines, provider calls, payments, or duplicate EMD architecture.</p>
            </div>
          ...
          {state?.emds?.length ? <EmdList emds={state.emds} showAgency /> : <EmptyState title="No EMD workspaces" body="EMD workspace metadata will appear here after platform records are created." />}
        </div>
      </ProtectedRoute>
    </PlatformLayout>
  )
...
function EmdList({ emds, showAgency = false }) {
  return (
        ...
        <tbody className="divide-y divide-slate-100">
          {emds.map((emd) => (
            <tr key={emd.id}>
              ...
              {showAgency ? <td className="px-4 py-3 align-top text-slate-700">{emd.agency_name || emd.agency_id}</td> : null}
