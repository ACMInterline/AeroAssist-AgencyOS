...
export default function PlatformFeatureBundleRolloutIssuesPage() {
  ...
  async function load(nextFilters = filters) {
    ...
    const [me, plans, issues] = await Promise.all([
      apiGet("/api/auth/me"),
      apiGet("/api/platform/feature-bundle-rollout-plans"),
      apiGet(`/api/platform/feature-bundle-rollout-issues${query}`),
    ])
    setState({
      me,
      plans: plans.items || [],
  ...
  }, [filters.agency_id, filters.bundle_id, filters.rollout_plan_id, filters.severity, filters.status])
        ...
        <div className="space-y-6">
          <div className="flex flex-wrap items-start justify-between gap-3">
            <div>
              ...
              <h2 className="mt-2 text-2xl font-semibold text-slate-950">Feature Bundle Rollout Issues</h2>
              <p className="mt-1 text-sm text-slate-600">Metadata-only rollout issue log. These records do not execute rollouts, activate bundles, enforce blocking, send notifications, call external providers, or add AI/provider execution.</p>
            </div>
          ...
          {state?.issues?.length ? <IssueTable issues={state.issues} /> : <EmptyState title="No rollout issues" body="Feature bundle rollout issue metadata will appear here after platform records are created." />}
        </div>
      </ProtectedRoute>
    </PlatformLayout>
  )
