...
export default function ReferenceDataEnginePage() {
  ...
  async function load(nextFilters = filters) {
    ...
    const [me, agencies, response] = await Promise.all([
      apiGet("/api/auth/me"),
      apiGet("/api/agencies"),
      apiGet(`/api/platform/reference-data-engine${query}`),
    ])
    setState({
      me,
      agencies: agencies.items || [],
      items: response.items || response.domains || [],
  ...
  }, [filters.agency_id, filters.domain_code, filters.governance_status, filters.review_status, filters.active, filters.search])
        ...
        <div className="space-y-6">
          <div className="flex flex-wrap items-start justify-between gap-3">
            <div>
              ...
              <h2 className="mt-2 text-2xl font-semibold text-slate-950">Reference Data Engine</h2>
              <p className="mt-1 text-sm text-slate-600">Metadata-only reference domains for airline operational knowledge production, normalization, validation, and governed review.</p>
            </div>
          ...
          <section className="space-y-3">
            ...
            {items.length ? <DomainList items={items} showAgency /> : <EmptyState title="No reference data domains" body="Reference Data Engine metadata will appear here." />}
          </section>
        </div>
      </ProtectedRoute>
    </PlatformLayout>
...
function DomainList({ items, showAgency = false }) {
  return (
          ...
          <summary className="cursor-pointer list-none">
            <div className="grid gap-3 md:grid-cols-[1fr_190px_190px]">
              ...
              <div className="text-xs text-slate-600">
                ...
                <p className="mt-1">Domain: {formatType(item.domain_code)}</p>
              </div>
              <div className="text-xs text-slate-600">
                <p>Governance: {formatType(item.governance_status)}</p>
