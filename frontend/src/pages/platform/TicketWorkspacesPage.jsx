...
export default function PlatformTicketWorkspacesPage() {
  ...
  async function load(nextFilters = filters) {
    ...
    const [me, agencies, tickets] = await Promise.all([
      apiGet("/api/auth/me"),
      apiGet("/api/agencies"),
      apiGet(`/api/platform/ticket-workspaces${query}`),
    ])
    setState({
      me,
  ...
  }, [filters.agency_id, filters.status, filters.document_status, filters.validating_carrier, filters.issue_date, filters.passenger, filters.booking_reference, filters.currency])
  ...
  const metrics = [
    ...
    ["Carriers", state?.summary?.by_validating_carrier ? Object.keys(state.summary.by_validating_carrier).length : 0],
  ]
        ...
        <div className="space-y-6">
          <div className="flex flex-wrap items-start justify-between gap-3">
            <div>
              ...
              <h2 className="mt-2 text-2xl font-semibold text-slate-950">Ticket Workspaces</h2>
              <p className="mt-1 text-sm text-slate-600">Metadata-only ticket workspace records. These records do not issue, reissue, void, refund, exchange, process payments, connect to GDS or NDC, call airline APIs, calculate fares, validate coupons, run background workers, or integrate external providers.</p>
            </div>
            <div className="flex flex-wrap gap-2">
              ...
              <span className="rounded-full bg-slate-100 px-3 py-2 text-xs font-semibold text-slate-700">No issuance</span>
            </div>
          </div>
          ...
          {state?.tickets?.length ? <TicketList tickets={state.tickets} showAgency /> : <EmptyState title="No ticket workspaces" body="Ticket workspace metadata will appear here after platform records are created." />}
        </div>
      </ProtectedRoute>
    </PlatformLayout>
  )
...
function TicketList({ tickets, showAgency = false }) {
  return (
        ...
        <tbody className="divide-y divide-slate-100">
          {tickets.map((ticket) => (
            <tr key={ticket.id}>
              ...
              {showAgency ? <td className="px-4 py-3 align-top text-slate-700">{ticket.agency_name || ticket.agency_id}</td> : null}
