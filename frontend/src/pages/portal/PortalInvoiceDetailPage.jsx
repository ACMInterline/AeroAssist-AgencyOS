    ...
    <ClientPortalLayout user={{ full_name: state?.me?.portal_account?.display_name }} brand={state?.me?.brand}>
      <ProtectedRoute loading={!state && !error} error={error}>
        {state ? <div className="space-y-6">
          ...
          <section className="grid gap-4 md:grid-cols-4"><Metric label="Total" value={money(state.invoice.total_amount, state.invoice.currency)} /><Metric label="Paid" value={money(state.invoice.paid_amount, state.invoice.currency)} /><Metric label="Due" value={money(state.invoice.due_amount, state.invoice.currency)} /><Metric label="Due date" value={state.invoice.due_date || "Not set"} /></section>
          <Panel title="Line items"><Rows items={state.line_items} render={(item) => `${item.description} · ${money(item.total_amount, item.currency)}`} /></Panel>
          <Panel title="Payments"><div className="mt-4 divide-y divide-slate-100 rounded-md border border-slate-200">{state.payments.length ? state.payments.map((item) => <div className="flex items-center justify-between gap-2 p-3 text-sm text-slate-700" key={item.id}><span>{money(item.amount, item.currency)} · {item.method?.replaceAll("_", " ")}</span><PaymentStatusBadge status={item.status} /></div>) : <div className="p-3 text-sm text-slate-500">No payments recorded.</div>}</div></Panel>
        </div> : null}
      </ProtectedRoute>
    </ClientPortalLayout>
  )
...
function money(value, currency) { return `${Number(value || 0).toFixed(2)} ${currency || ""}`.trim() }
