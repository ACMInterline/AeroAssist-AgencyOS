...
function ListPage({ title, body, items, href, render }) {
  return <div className="space-y-6"><div className="flex flex-wrap items-end justify-between gap-3"><div><p className="text-sm font-semibold uppercase tracking-wide text-blue-700">Client portal</p><h2 className="mt-2 text-2xl font-semibold text-slate-950">{title}</h2><p className="mt-1 text-sm text-slate-600">{body}</p></div><a className="rounded-md bg-blue-600 px-4 py-2 text-sm font-semibold text-white" href="/portal/requests/new">New request</a></div>{!items.length ? <EmptyState title={`No ${title.toLowerCase()} visible`} body="Your agency controls which records are visible here." /> : <div className="divide-y divide-slate-100 rounded-lg border border-slate-200 bg-white">{items.map((item) => <a className="block p-4 text-sm hover:bg-slate-50" href={`${href}/${item.id}`} key={item.id}>{render(item)}</a>)}</div>}</div>
}
