...
| Focused CI | The first Focused run that reached inventory execution failed because its selected UX smoke invokes the frontend build but the job had not installed the locked frontend toolchain | require Node 20 setup, `npm ci --prefix frontend` before both inventory tiers, zero-warning actionlint, and a successful hosted Focused rerun without skipping the UX smoke |
