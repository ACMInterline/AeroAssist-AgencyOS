...
Phase 52.6 adds `knowledge_quality_assurance_reviews` as a QA review layer over policy-card metadata. Knowledge QA can record missing evidence, conflicting support status, duplicate policy card, missing documents, or requested changes, but it does not auto-approve cards, publish policy, execute rules, use AI, call providers, run workers, or replace human authority.

Phase 52.7 adds Airline Knowledge Publishing through `airline_knowledge_publications` as controlled publication workflow metadata. Knowledge Publishing can include approved policy-card references, QA review links, release channels, consumer readiness, and agency visibility metadata, but it does not publish automatically, execute recommendations, call providers, use AI, run workers, or replace human authority.
