...
router = APIRouter(prefix="/api/agencies/{agency_id}/travel-request-workspaces", tags=["agency-travel-request-workspaces"])
...
@router.get("")
async def list_agency_travel_request_workspaces(
    agency_id: str,
    ...
    return await TravelRequestWorkspaceService(db).agency_response(
        agency_id,
...
@router.get("/summary")
async def summarize_agency_travel_request_workspaces(
    agency_id: str,
    user: dict = Depends(get_current_user),
    db: Database = Depends(get_database),
) -> dict:
    await require_agency_read(db, agency_id, user)
    return await TravelRequestWorkspaceService(db).agency_summary(agency_id)


@router.get("/{request_workspace_id}")
async def get_agency_travel_request_workspace(
    agency_id: str,
    ...
    try:
        request_workspace = await service.get_agency_request(agency_id, request_workspace_id)
