...
router = APIRouter(prefix="/api/agencies/{agency_id}/feature-bundle-rollout-risks", tags=["agency-feature-bundle-rollout-risks"])
...
@router.get("")
async def list_agency_feature_bundle_rollout_risks(
    agency_id: str,
    ...
    return await FeatureBundleRolloutRiskService(db).agency_response(
        agency_id,
        bundle_id=bundle_id,
...
@router.get("/summary")
async def summarize_agency_feature_bundle_rollout_risks(
    agency_id: str,
    user: dict = Depends(get_current_user),
    db: Database = Depends(get_database),
) -> dict:
    await require_agency_read(db, agency_id, user)
    return await FeatureBundleRolloutRiskService(db).agency_summary(agency_id)


@router.get("/{risk_id}")
async def get_agency_feature_bundle_rollout_risk(
    agency_id: str,
    risk_id: str,
    user: dict = Depends(get_current_user),
    db: Database = Depends(get_database),
    ...
    try:
        risk = await service.get_agency_risk(agency_id, risk_id)
