...
router = APIRouter(prefix="/api/agencies/{agency_id}/feature-bundle-rollout-summary-packs", tags=["agency-feature-bundle-rollout-summary-packs"])
...
@router.get("")
async def list_agency_feature_bundle_rollout_summary_packs(
    agency_id: str,
    ...
    return await FeatureBundleRolloutSummaryPackService(db).agency_response(
        agency_id,
...
@router.get("/summary")
async def summarize_agency_feature_bundle_rollout_summary_packs(
    agency_id: str,
    user: dict = Depends(get_current_user),
    db: Database = Depends(get_database),
) -> dict:
    await require_agency_read(db, agency_id, user)
    return await FeatureBundleRolloutSummaryPackService(db).agency_summary(agency_id)


@router.get("/{pack_id}")
async def get_agency_feature_bundle_rollout_summary_pack(
    agency_id: str,
    pack_id: str,
    user: dict = Depends(get_current_user),
    db: Database = Depends(get_database),
    ...
    try:
        summary_pack = await service.get_agency_summary_pack(agency_id, pack_id)
