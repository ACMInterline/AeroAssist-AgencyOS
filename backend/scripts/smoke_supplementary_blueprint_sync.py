...
def main() -> int:
    ...
    if "Phase 50.5" not in gaps.get("next_immediate_phase", ""):
        raise AssertionError("Gap summary did not identify Phase 50.5 Operational Capability Matrix Foundation as next.")
    if "Phase 50.5" not in gaps.get("next_intelligence_phase", ""):
        raise AssertionError("Gap summary did not identify Phase 50.5 as the next intelligence phase.")
    ...
    if not next_phases.get("items") or next_phases["items"][0].get("phase") != "Phase 50.5":
        raise AssertionError("Next phase recommendations did not start with Phase 50.5.")
