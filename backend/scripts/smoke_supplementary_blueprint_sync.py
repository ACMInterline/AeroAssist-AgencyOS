...
EXPECTED_PHASE = "phase_52_9_knowledge_population_toolkit_foundation"
REQUIRED_CATEGORIES = {
    "RBAC",
    "Airline Intelligence",
    "Airline Operational Intelligence Engine",
    ...
    "Feature Bundle Assignments",
    "Feature Bundle Dependencies",
    "Feature Bundle Rollout Risk Register",
    "Feature Bundle Rollout Issue Log",
    "Feature Bundle Rollout Decision Register",
    "Feature Bundle Rollout Change Requests",
    "Feature Bundle Rollout Rollback Plans",
    "Feature Bundle Rollout Summary Packs",
...
def main() -> int:
    ...
    if "Phase 50.9" not in gaps.get("next_immediate_phase", ""):
        raise AssertionError("Gap summary did not identify Phase 50.9 Offer Builder Intelligence Integration Foundation as next.")
    if "Phase 50.9" not in gaps.get("next_intelligence_phase", ""):
        raise AssertionError("Gap summary did not identify Phase 50.9 as the next intelligence phase.")
    ...
    if not any("Operational timeline workspace foundation built in Phase 42.1" in item for item in gaps.get("already_built", [])):
        raise AssertionError("Gap summary did not recognize Phase 42.1 Operational timeline workspace foundation as already built.")
    if not any("Passenger service workflow engine foundation built in Phase 42.2" in item for item in gaps.get("already_built", [])):
        raise AssertionError("Gap summary did not recognize Phase 42.2 Passenger service workflow engine foundation as already built.")
    ...
    if not any("Airline knowledge acquisition workspace foundation built in Phase 50.1" in item for item in gaps.get("already_built", [])):
        raise AssertionError("Gap summary did not recognize Phase 50.1 Airline Knowledge Acquisition foundation as already built.")
    ...
    if not next_phases.get("items") or next_phases["items"][0].get("phase") != "Phase 50.9":
        raise AssertionError("Next phase recommendations did not start with Phase 50.9.")
