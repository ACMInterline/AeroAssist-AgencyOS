...
def verify_blueprint_adoption() -> None:
    ...
    if "Phase 42.0" not in gaps.get("next_operational_phase", ""):
        raise AssertionError(f"Blueprint gaps missing Phase 42.0 operational marker: {gaps}")
    ...
    if not any(item.get("phase") == "Phase 42.0" for item in next_phases.get("items", [])):
        raise AssertionError(f"Next recommendations missing Phase 42.0: {next_phases}")
