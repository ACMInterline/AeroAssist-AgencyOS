...
def verify_frontend_and_docs() -> None:
    for path, text in [
        ...
        (ROOT / "docs/architecture/supplementary-blueprint-adoption-map.md", "Phase 50.1 - Airline Knowledge Acquisition Workspace"),
    ]:
...
def verify_blueprint_recommendations() -> None:
    ...
    if not any(item.get("phase") == "Phase 41.9" for item in next_phases.get("items", [])):
        raise AssertionError(f"Next recommendations did not preserve Phase 41.9: {next_phases}")
