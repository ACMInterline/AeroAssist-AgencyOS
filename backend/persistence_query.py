...
COLLECTION_OWNERSHIP_REGISTRY: dict[str, CollectionOwnership] = {
    ...
    for item in (
        ...
        _agency("operational_timelines", filters=("entity_type", "entity_id", "event_type", "event_category", "event_status", "event_priority", "communication_type", "visibility", "linked_communication_thread_id", "linked_audit_event_id", "idempotency_key", "passenger_workspace_id", "booking_workspace_id", "ticket_workspace_id", "emd_workspace_id", "ssr_osi_workspace_id", "related_airline"), sorts=("event_time", "created_at", "event_priority", "event_status"), ownership=CollectionOwnershipType.AUDIT_SECURITY, historical_reads=True, recommended_indexes=("operational_timelines_agency_created_id", "operational_timelines_agency_entity_event_time_lookup")),
        _agency("communication_threads", filters=("status", "participant_ids"), sorts=("updated_at", "status"), historical_reads=True, recommended_indexes=("communication_threads_agency_status_updated_lookup",)),
        _agency("communication_messages", filters=("thread_id", "visibility", "sender_identity_id", "message_type", "linked_timeline_entry_id"), sorts=("created_at", "visibility"), historical_reads=True, recommended_indexes=("communication_messages_agency_thread_created_lookup",)),
        _agency("communication_participants", filters=("thread_id", "participant_type", "identity_id", "client_id", "passenger_id", "status"), sorts=("created_at", "status")),
        _agency("communication_attachments", filters=("thread_id", "message_id", "reference_type", "reference_id", "document_id", "visibility"), sorts=("created_at",)),
        _agency("operational_notification_projections", filters=("timeline_entry_id", "participant_id", "notification_type", "status", "visibility"), sorts=("created_at", "status"), ownership=CollectionOwnershipType.OPERATIONAL_EPHEMERAL, recommended_indexes=("operational_notification_projections_agency_status_created_lookup",)),
        _agency("operational_work_items", filters=("queue_code", "status", "priority", "severity", "work_item_type", "source_entity_type", "source_entity_id", "assigned_user_id", "assigned_team_code", "blocker_status", "sla_status"), sorts=("priority", "severity", "due_at", "status"), ownership=CollectionOwnershipType.OPERATIONAL_EPHEMERAL, recommended_indexes=("operational_work_items_agency_status_due_created_id",)),
...
GOVERNED_INDEX_SPECS: tuple[dict[str, Any], ...] = (
    ...
    {"collection": "operational_timelines", "name": "operational_timelines_agency_entity_event_time_lookup", "keys": (("agency_id", 1), ("entity_type", 1), ("entity_id", 1), ("event_time", 1)), "rationale": "canonical entity activity"},
    {"collection": "communication_threads", "name": "communication_threads_agency_status_updated_lookup", "keys": (("agency_id", 1), ("status", 1), ("updated_at", -1)), "rationale": "tenant communication thread list"},
    {"collection": "communication_messages", "name": "communication_messages_agency_thread_created_lookup", "keys": (("agency_id", 1), ("thread_id", 1), ("created_at", 1), ("id", 1)), "rationale": "thread message history"},
    {"collection": "operational_notification_projections", "name": "operational_notification_projections_agency_status_created_lookup", "keys": (("agency_id", 1), ("status", 1), ("created_at", -1)), "rationale": "tenant notification projection"},
